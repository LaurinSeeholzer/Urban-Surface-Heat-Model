...
class Simulation {
    ...
    run(params) {
        ...
        for (let time = 0; time < params.iterations; time++) {
            ...
            if ((time+1)%10 === 0) {
                ...
                this.informFrontend(params, 'statusUpdate', data);
            }
        } 
    ...
    applyHeatDiffusion(params, tempMap) {
        let newTemp = Array.from({ length: params.pointsX }, () => Array(params.pointsY).fill(1));
        for (let x = 0; x < params.pointsX; x++) {
            for (let y = 0; y < params.pointsY; y++) {
                ...
                newTemp[x][y] = tempMap[x][y] + (thermalDiffusivity * params.deltaTime * (tempMap[nx][y] + tempMap[px][y] + tempMap[x][ny] + tempMap[x][py] - 4 * tempMap[x][y]) / (params.deltaX ** 2));
            }
        }
        tempMap = newTemp
    }

    //calculates the minimal temperature of given tempMap
    calculateMinimalTemperture(tempMap) {
        let minTemp = Math.min(...tempMap.map(row => Math.min(...row)));
        return minTemp;
    }

    //calculates maximum temperature of given tempMap
